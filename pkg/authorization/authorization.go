...
// and adapted to this use case, ideally this should be simplified and all the istio related stuff removed.
//
...
func (a *Authorizer) AuthRep(request AuthorizeRequest) (backendC.ApiResponse, error) {
	...
	if len(m) == 0 {
		return backendC.ApiResponse{Success:false}, fmt.Errorf("path not allowed")
	}
	...
	resp, err := authRep(authRepRequest.auth, authRepRequest.authKey, params.ServiceId, authRepRequest.params, nil)
